...
<div class="container">
	<div class="page_content">
		<?php
		$podcasts_id = get_the_ID();
		$banner_url = get_post_meta( $podcasts_id, 'banner-url', true );

		if ( ! empty( $banner_url ) ) {
			$banner_src = wp_get_attachment_image_src( get_post_meta( $podcasts_id, 'banner-image', true ), 'large' )[0];
			printf(
				'<a target="_blank" href="%1$s">
					<img class="podcasts-header-image" src="%2$s">
				</a>',
				esc_url( $banner_url ),
				esc_url( $banner_src )
			);
